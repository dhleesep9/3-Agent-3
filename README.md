...
3. main 으로 풀리퀘 날려주시면 됩니다. merge 는 하시면 안돼요! 풀리퀘 제목 : 3기_11주차_[이름]

### 예시

```bash
git clone git@github.com:HateSlop/3-Agent-3.git  # 클론
cd 3-Agent-3 # 프로젝트 루트로 이동
...
```bash
.
├── README.md               # 프로젝트 설명 리드미 (수정 금지)
├── 3-agent-3.ipynb         # 실습 파일
├── try/                    
│   ├── data/               # 데이터 폴더
│   │   ├── data1.pkl
│   │   ├── data2.pkl
...
│   ├── Dockerfile          # 컨테이너 실행 파일
│   └── docker-compose.yml  # compose 실행 설정
└── 본인_폴더명/              # 개인 작업물
    └── (과제)
